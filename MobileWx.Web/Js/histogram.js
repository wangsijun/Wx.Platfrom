...
(function (){
	...
	SerialChart.prototype.addTo = function(container){
		...
		if(this.type==="column"){
            ...
            var minValue=0;
            var temp=Math.min.apply(Math,values);
            if (temp<0) {
                minValue=(temp/m)*-1;
            }
            ...
            if (minValue!=0) {
                minValue=(Math.floor(minValue)+1)*m;
            }
             
            var top=23;
            var foot=22;
            //柱状图区域高度
			var h = ca.height-top-foot; 
             
			//坐标放大系数
			var zoomK = h/(maxValue+minValue); 
            //X轴高度
            var xLineY=ca.height-top-(minValue * zoomK);
            
			var w = ca.width - 10;
			
			var eachSegX = Math.floor(w/(columns));
			var sX = eachSegX;
			var k=0;
			while(w>=0){
				...
				if(w>=0){ 
                    //文字
				    ca.drawText({
				        message: this.dataProvider[k][this.categoryField],
				        textBaseLine: "middle",
				        textAlign: "center ",
				        font: {
				            fontWeight: "normal",
				    ...
				    if (this.columnShadow)
				    {
				        fi.shadow = { blur: 10, color: "rgba(100,100,100,0.5)", offsetX: 0, offsetY: 0 };
				    }
                    //画主图 x,y,w,h
					ca.drawRect((sX * k) + 15, xLineY, this.columnWidth, values[k] * zoomK * -1).fill(fi);
				    //标签
					if (this.labelField != null) {
					    ca.drawText({
					        message: this.dataProvider[k][this.labelField],
					        textBaseLine: "middle",
					        textAlign: "center ",
					        font: {
					            fontWeight: "normal",
					        ...
					        color: "#909090",
					        xPos: (sX * k) + 15,
					        yPos: xLineY + (values[k] * zoomK * -1) - 10,
					        type: "fill"
            ...
            //X轴
			ca.drawLine(10,xLineY,ca.width-15,xLineY).stroke({color:"#B70D00"});
